...
pct_nolow <- paste(round(prop.table(table(as.character(data$Insecurity_level)))[1]*100, 1), "%", sep = "")
pct_mild <- paste(round(prop.table(table(as.character(data$Insecurity_level)))[2]*100, 1), "%", sep = "")
pct_moderate <- paste(round(prop.table(table(as.character(data$Insecurity_level)))[3]*100, 1), "%", sep = "")
pct_severe <- paste(round(prop.table(table(as.character(data$Insecurity_level)))[4]*100, 1), "%", sep = "")
...
Insecurity_by_HHsize <- ggplot(hhsize_data, aes(x = HHsize_group, y = percentage, fill = Insecurity_level)) +
    ...
    scale_fill_manual(values = custom_colors,
                      labels = c("Severe", 
                                 "Moderate", 
                                 "Mild", 
                                 "No-to-low")) +  # You can change the color palette as needed
...
grid.text(paste(
    ...
    "ienced moderate-to-high water insecurity in 2023. ", pct_severe, "\n",
    "of respondents' water insecurity experiences were classified", "\n",
    "as 'severe,' while ", pct_moderate, " of respondents were considered", "\n",
    "'moderately' insecure. ", pct_mild, " were considered 'mildly' insecure,", "\n",
    "while ", pct_nolow, " experienced no-to-low water insecurity.",
    sep = ""), vjust = 0, hjust = 0, 
    x = unit(0.05, "npc"), 
    y = unit(0.45, "npc"), 
    gp = gpar(col = "Black", cex = 1))
